...
/// states. A `bool` cannot carry them: `Trails` separates a value that closed
/// across RFC 9110 §5.2's join from one that closed and then ran on past its
/// close.
#[inline]
pub fn auth_param(
  element: &[u8],
...
/// three states, and mirrors them as an ENUM: a `u8` here would put values at
/// the boundary that name no state at all and need a fallback arm to swallow
/// them, which is three states and a hole where the type should be.
///
