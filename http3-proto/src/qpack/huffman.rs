...
// Transient: `CODES`, `decode`, and `encoded_len` are consumed by the QPACK
// field-section decoder/encoder in later tasks; until a production caller exists
// they are reachable only from tests, so dead_code is silenced for now.
#![allow(dead_code)]

use super::QpackError;
use crate::error::BufferTooSmallDetail;

/// `(code, bit_length)` per symbol — indices 0..=255 are bytes, index 256 is
/// EOS. Transcribed verbatim from RFC 7541 Appendix B (validated as a complete
/// canonical prefix code: Kraft sum == 2^30 and the code ranges tile [0, 2^30)).
...
/// The first symbol index (0..=256) whose code matches `(acc, nbits)`, if any.
#[inline]
fn lookup(acc: u32, nbits: u32) -> Option<usize> {
  CODES
    .iter()
    .position(|&(code, len)| u32::from(len) == nbits && code == acc)
}
...
pub fn encoded_len(input: &[u8]) -> usize {
  let bits = input.iter().fold(0usize, |acc, &b| {
    acc.saturating_add(
      CODES
        .get(usize::from(b))
        .map_or(0, |&(_, len)| usize::from(len)),
    )
  });
  bits.div_ceil(8)
}
...
pub fn decode(input: &[u8], out: &mut [u8]) -> Result<usize, QpackError> {
  ...
  for &byte in input {
    for shift in (0..8u32).rev() {
      ...
      if let Some(sym) = lookup(acc, nbits) {
        if sym == 256 {
