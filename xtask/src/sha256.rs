...
mod tests {
  ...
  /// FIPS 180-4's published examples, and the two boundaries between them.
  ///
  /// Run rather than claimed, because every digest this workspace publishes
  /// over its differential corpus is taken by the code below. A hash that had
  /// drifted would move those digests together, so the pins that stand on them
  ...
  /// FIPS 180-4 §B.2's two-block message, 896 bits — the vector that crosses a
  /// block boundary in the middle of the message rather than in the padding.
  const TWO_BLOCK: (&str, &str) = (
  ...
  #[test]
  fn the_published_vectors_are_the_digests_this_answers() {
    for (message, want) in VECTORS {
      assert_eq!(digest(message.as_bytes()), want, "{message:?}");
    }
    let (message, want) = TWO_BLOCK;
    assert_eq!(digest(message.as_bytes()), want, "the two-block message");

    // The long one, which is where a length counter that overflowed a `u32`
    // or a padding length taken after the padding would first disagree.
    let mut hash = Sha256::new();
    for _ in 0..1_000 {
  ...
  #[test]
  fn a_message_fed_in_pieces_digests_as_the_whole_does() {
    // Every caller in this crate feeds the hash a piece at a time — a line and
    // its newline — so the buffering is the part they all rely on and none of
    // them exercises alone. Over every split of a message longer than two
    // blocks, and over one-byte pieces, which is the split that fills the
    // buffer at every offset in it.
    let (message, want) = TWO_BLOCK;
    let message = message.as_bytes();
    for cut in 0..=message.len() {
      let mut hash = Sha256::new();
      hash.update(message.get(..cut).unwrap_or_default());
      hash.update(message.get(cut..).unwrap_or_default());
      assert_eq!(hash.finish(), want, "split at {cut}");
    }
    ...
    assert_eq!(hash.finish(), want, "one byte at a time");
  }
}
